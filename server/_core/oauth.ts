...
import express, { type Express, type Request, type Response } from "express"; // 'type' ekleyerek import et
...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    if (!code || !state) {
      (res as any).status(400).json({ error: "code and state are required" }); // 'as any' zorlaması yap
      return;
    }
